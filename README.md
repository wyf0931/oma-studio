...
- Generated-file drawer for Chat sessions, with metadata cards, a new-tab Markdown viewer with Mermaid support, and independently revocable public Markdown artifact links.
...
| `GET /api/chats/{id}/share` | Read an existing public share token for a chat |
| `GET /api/chats/{id}/artifact-shares?path=` | Read an existing public share for one generated Markdown artifact |
| `POST /api/chats/{id}/artifact-shares?path=` | Create/reuse an unguessable public share for one generated Markdown artifact |
| `GET /api/shares` | List the current user's active session and artifact shares |
| `DELETE /api/shares/{token}` | Revoke one of the current user's session or artifact share links |
...
| `GET /api/share/{token}/files/content?path=` | Read one shared chat file, or only the exact Markdown artifact granted by an artifact token (same `resolve_chat_file` authorization) |
| `GET /api/share/{token}/files/view?path=` | Open a supported shared HTML, image, or PDF file inline; generated HTML and well-formed SVG are sandboxed, malformed SVG falls back to source |
